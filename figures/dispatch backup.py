...
class Dispatch:
    ...
    def __init__(self,T,a,c,Q0,dQ0,dQT,S):
        if a<0 or c<0:
        ...
        if c == 0:
            ...
            if S == None:
                ...
                l = 6*c/T**2*(S-Q0*T-dQ0/2*T**2)
            else:
                raise Exception("both S and dQT cannot be constrained when a=0")
            w = float('nan')
            pass
        else:
            w = sqrt(a/c)
            l = c*T*(dQT-w*Q0*sinh(w*T)-dQ0*cosh(w*T))/sinh(w*T)
